 ...
 * L'objet est relaché quand la variable R3F_LOG1_joueur_deplace_objet passe à objNull ce qui terminera le script
 * 
...
else
{
	...
	if (isNull (_objet getVariable ["R3F_LOG1_est_transporte_par", objNull]) && (isNull (_objet getVariable ["R3F_LOG1_est_deplace_par", objNull]) || (!alive (_objet getVariable ["R3F_LOG1_est_deplace_par", objNull])) || (!isPlayer (_objet getVariable ["R3F_LOG1_est_deplace_par", objNull])))) then
	{
		if (isNull (_objet getVariable ["R3F_LOG1_remorque", objNull])) then
		{
			if (count crew _objet == 0 || getNumber (configFile >> "CfgVehicles" >> (typeOf _objet) >> "isUav") == 1) then
			{
				[_objet, _joueur] call R3F_LOG1_FNCT_definir_proprietaire_verrou;
				
				_objet setVariable ["R3F_LOG1_est_deplace_par", _joueur, true];
				
				_joueur forceWalk true;
				
				R3F_LOG1_joueur_deplace_objet = _objet;
				...
				if (_decharger) then
				{
					...
					{R3F_LOG1_deplace_dir_rel_objet = 90;} else {R3F_LOG1_deplace_dir_rel_objet = 0;};
					...
					_pos_rel_objet_initial = [
						(boundingCenter _objet select 0) * cos R3F_LOG1_deplace_dir_rel_objet - (boundingCenter _objet select 1) * sin R3F_LOG1_deplace_dir_rel_objet,
						((-(boundingBoxReal _objet select 0 select 0) * sin R3F_LOG1_deplace_dir_rel_objet) max (-(boundingBoxReal _objet select 1 select 0) * sin R3F_LOG1_deplace_dir_rel_objet)) +
						((-(boundingBoxReal _objet select 0 select 1) * cos R3F_LOG1_deplace_dir_rel_objet) max (-(boundingBoxReal _objet select 1 select 1) * cos R3F_LOG1_deplace_dir_rel_objet)) +
						2 + 0.3 * (
							((boundingBoxReal _objet select 1 select 1)-(boundingBoxReal _objet select 0 select 1)) * abs sin R3F_LOG1_deplace_dir_rel_objet +
							((boundingBoxReal _objet select 1 select 0)-(boundingBoxReal _objet select 0 select 0)) * abs cos R3F_LOG1_deplace_dir_rel_objet
						),
				...
				{
					R3F_LOG1_deplace_dir_rel_objet = (getDir _objet) - _dir_joueur;
					...
					// L'ajout de ce calcul a également rendu inutile le test avec la fonction R3F_LOG1_FNCT_unite_marche_dessus lors de la prise de l'objet
					_pos_rel_objet_initial = [
				...
				R3F_LOG1_deplace_distance_rel_objet = _pos_rel_objet_initial select 1;
				
				// Détermination du mode d'alignement initial en fonction du type d'objet, de ses dimensions, ...
				R3F_LOG1_deplace_mode_alignement = switch (true) do
				{
				...
				if (!local _objet) then
				{
					...
					[_objet, "setOwnerTo", _joueur] call R3F_LOG1_FNCT_exec_commande_MP;
				...
				R3F_LOG1_PV_nouvel_objet_en_deplacement = _objet;
				publicVariable "R3F_LOG1_PV_nouvel_objet_en_deplacement";
				["R3F_LOG1_PV_nouvel_objet_en_deplacement", R3F_LOG1_PV_nouvel_objet_en_deplacement] call R3F_LOG1_FNCT_PVEH_nouvel_objet_en_deplacement;
				...
				if (_objet getVariable "R3F_LOG1_est_deplace_par" == _joueur && isNull (_objet getVariable ["R3F_LOG1_est_transporte_par", objNull])) then
				{
					R3F_LOG1_deplace_force_setVector = false; // Mettre à true pour forcer la ré-otientation de l'objet, en forçant les filtres anti-flood
					R3F_LOG1_deplace_force_attachTo = false; // Mettre à true pour forcer le repositionnement de l'objet, en forçant les filtres anti-flood
					
					// Ajout des actions de gestion de l'orientation
					_action_relacher = _joueur addAction [("<t color=""#ee0000"">" + format [STR_R3F_LOG1_action_relacher_objet, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")] + "</t>"), {_this call R3F_LOG1_FNCT_objet_relacher}, nil, 10, true, true];
					_action_aligner_pente = _joueur addAction [("<t color=""#00eeff"">" + STR_R3F_LOG1_action_aligner_pente + "</t>"), {R3F_LOG1_deplace_mode_alignement = "pente"; R3F_LOG1_deplace_force_setVector = true;}, nil, 6, false, true, "", "R3F_LOG1_deplace_mode_alignement != ""pente"""];
					_action_aligner_sol = _joueur addAction [("<t color=""#00eeff"">" + STR_R3F_LOG1_action_aligner_sol + "</t>"), {R3F_LOG1_deplace_mode_alignement = "sol"; R3F_LOG1_deplace_force_setVector = true;}, nil, 6, false, true, "", "R3F_LOG1_deplace_mode_alignement != ""sol"""];
					_action_aligner_horizon = _joueur addAction [("<t color=""#00eeff"">" + STR_R3F_LOG1_action_aligner_horizon + "</t>"), {R3F_LOG1_deplace_mode_alignement = "horizon"; R3F_LOG1_deplace_force_setVector = true;}, nil, 6, false, true, "", "R3F_LOG1_deplace_mode_alignement != ""horizon"""];
					_action_tourner = _joueur addAction [("<t color=""#00eeff"">" + STR_R3F_LOG1_action_tourner + "</t>"), {R3F_LOG1_deplace_dir_rel_objet = R3F_LOG1_deplace_dir_rel_objet + 12; R3F_LOG1_deplace_force_setVector = true;}, nil, 6, false, false];
					_action_rapprocher = _joueur addAction [("<t color=""#00eeff"">" + STR_R3F_LOG1_action_rapprocher + "</t>"), {R3F_LOG1_deplace_distance_rel_objet = R3F_LOG1_deplace_distance_rel_objet - 0.4; R3F_LOG1_deplace_force_attachTo = true;}, nil, 6, false, false];
					
					// Relâcher l'objet dès que le joueur tire. Le detach sert à rendre l'objet solide pour ne pas tirer au travers.
					_idx_eh_fired = _joueur addEventHandler ["Fired", {if (!surfaceIsWater getPos player) then {detach R3F_LOG1_joueur_deplace_objet; R3F_LOG1_joueur_deplace_objet = objNull;};}];
					
					// Gestion des évènements KeyDown et KeyUp pour faire tourner l'objet avec les touches X/C
					R3F_LOG1_joueur_deplace_key_rotation = "";
					R3F_LOG1_joueur_deplace_key_translation = "";
					...
					_idx_eh_keyDown = (findDisplay 46) displayAddEventHandler ["KeyDown",
					{
						...
						{
							case 45: {R3F_LOG1_joueur_deplace_key_rotation = "X"; true};
							case 46: {R3F_LOG1_joueur_deplace_key_rotation = "C"; true};
							case 33: {R3F_LOG1_joueur_deplace_key_translation = "F"; true};
							case 19: {R3F_LOG1_joueur_deplace_key_translation = "R"; true};
							default {false};
						}
					}];
					_idx_eh_keyUp = (findDisplay 46) displayAddEventHandler ["KeyUp",
					{
						switch (_this select 1) do
						{
							case 45: {R3F_LOG1_joueur_deplace_key_rotation = ""; true};
							case 46: {R3F_LOG1_joueur_deplace_key_rotation = ""; true};
							case 33: {R3F_LOG1_joueur_deplace_key_translation = ""; true};
							case 19: {R3F_LOG1_joueur_deplace_key_translation = ""; true};
							default {false};
						}
					}];
					...
					_vec_dir_rel = [sin R3F_LOG1_deplace_dir_rel_objet, cos R3F_LOG1_deplace_dir_rel_objet, 0];
					...
					if (!local _objet) then {[_objet, "setDir", R3F_LOG1_deplace_dir_rel_objet] call R3F_LOG1_FNCT_exec_commande_MP;};
					
					R3F_LOG1_mutex_local_verrou = false;
					
					// Boucle de gestion des évènements et du positionnement pendant le déplacement
					while {!isNull R3F_LOG1_joueur_deplace_objet && _objet getVariable "R3F_LOG1_est_deplace_par" == _joueur && alive _joueur} do
					{
						// Gestion de l'orientation de l'objet en fonction du terrain
						if (local _objet) then
						{
							// En fonction de la touche appuyée (X/C), on fait pivoter l'objet
							if (R3F_LOG1_joueur_deplace_key_rotation == "X" || R3F_LOG1_joueur_deplace_key_rotation == "C") then
							{
								// Un cycle sur deux maxi (flood) on modifie de l'angle
								if (time - _time_derniere_rotation > 0.045) then
								{
									if (R3F_LOG1_joueur_deplace_key_rotation == "X") then {R3F_LOG1_deplace_dir_rel_objet = R3F_LOG1_deplace_dir_rel_objet + 4;};
									if (R3F_LOG1_joueur_deplace_key_rotation == "C") then {R3F_LOG1_deplace_dir_rel_objet = R3F_LOG1_deplace_dir_rel_objet - 4;};
									
									R3F_LOG1_deplace_force_setVector = true;
							...
							_vec_dir_up = switch (R3F_LOG1_deplace_mode_alignement) do
							{
								case "sol": {[[-cos R3F_LOG1_deplace_dir_rel_objet, sin R3F_LOG1_deplace_dir_rel_objet, 0] vectorCrossProduct _normale_surface, _normale_surface]};
								case "pente": {[_vec_dir_rel, _normale_surface]};
								default {[_vec_dir_rel, [0, 0, 1]]};
							};
							
							// On ré-oriente l'objet, lorsque nécessaire (pas de flood)
							if (R3F_LOG1_deplace_force_setVector ||
								(
							...
							) then
							{
								...
								R3F_LOG1_deplace_force_setVector = false;
							};
						};
						...
						if (R3F_LOG1_joueur_deplace_key_translation == "F" || R3F_LOG1_joueur_deplace_key_translation == "R") then
						{
							// Un cycle sur deux maxi (flood) on modifie de l'angle
							if (time - _time_derniere_translation > 0.045) then
							{
								if (R3F_LOG1_joueur_deplace_key_translation == "F") then
								{
									R3F_LOG1_deplace_distance_rel_objet = R3F_LOG1_deplace_distance_rel_objet - 0.075;
								}
								else
								{
									R3F_LOG1_deplace_distance_rel_objet = R3F_LOG1_deplace_distance_rel_objet + 0.075;
								...
								R3F_LOG1_deplace_distance_rel_objet = R3F_LOG1_deplace_distance_rel_objet min (
										(
											vectorMagnitude [
								...
								R3F_LOG1_deplace_force_attachTo = true;
						...
						_pos_rel_objet = [
							_pos_rel_objet_initial select 0,
							R3F_LOG1_deplace_distance_rel_objet max
							(
								((-(boundingBoxReal _objet select 0 select 0) * sin R3F_LOG1_deplace_dir_rel_objet) max (-(boundingBoxReal _objet select 1 select 0) * sin R3F_LOG1_deplace_dir_rel_objet)) +
								((-(boundingBoxReal _objet select 0 select 1) * cos R3F_LOG1_deplace_dir_rel_objet) max (-(boundingBoxReal _objet select 1 select 1) * cos R3F_LOG1_deplace_dir_rel_objet)) +
								1.2
						...
						if (_objet isKindOf "Static") then
						{
							...
							if (R3F_LOG1_deplace_mode_alignement == "horizon") then
							{
								_hauteur_terrain_min_max_objet = [_objet] call R3F_LOG1_FNCT_3D_get_hauteur_terrain_min_max_objet;
							...
							if (R3F_LOG1_CFG_no_gravity_objects_can_be_set_in_height_over_ground) then
							{
						...
						if (R3F_LOG1_deplace_force_attachTo ||
							(
					...
					if (isNull (_objet getVariable ["R3F_LOG1_est_transporte_par", objNull])) then
					{
						...
						[_objet, "detachSetVelocity", [0, 0, 0.1]] call R3F_LOG1_FNCT_exec_commande_MP;
					};
				...
				{
					if (isNull (_objet getVariable ["R3F_LOG1_est_deplace_par", objNull]) ||
						{(!alive (_objet getVariable "R3F_LOG1_est_deplace_par")) || (!isPlayer (_objet getVariable "R3F_LOG1_est_deplace_par"))}
					) then
					{
						R3F_LOG1_PV_fin_deplacement_objet = _objet;
						publicVariable "R3F_LOG1_PV_fin_deplacement_objet";
						["R3F_LOG1_PV_fin_deplacement_objet", R3F_LOG1_PV_fin_deplacement_objet] call R3F_LOG1_FNCT_PVEH_fin_deplacement_objet;
					};
				};
			}
			else
			{
				hintC format [STR_R3F_LOG1_joueur_dans_objet, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")];
				R3F_LOG1_mutex_local_verrou = false;
			};
		}
		else
		{
			hintC format [STR_R3F_LOG1_objet_remorque_en_cours, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")];
			R3F_LOG1_mutex_local_verrou = false;
		};
	}
	else
	{
		hintC format [STR_R3F_LOG1_objet_en_cours_transport, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")];
		R3F_LOG1_mutex_local_verrou = false;
	};
};