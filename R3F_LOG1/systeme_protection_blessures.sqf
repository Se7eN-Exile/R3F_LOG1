...
R3F_LOG1_liste_objets_en_deplacement = [];
 ...
 */
R3F_LOG1_FNCT_PVEH_nouvel_objet_en_deplacement =
{
	private ["_objet"];
	
	_objet = _this select 1;
	
	R3F_LOG1_liste_objets_en_deplacement = R3F_LOG1_liste_objets_en_deplacement - [_objet];
	R3F_LOG1_liste_objets_en_deplacement pushBack _objet;
	
	_objet allowDamage false;
};
"R3F_LOG1_PV_nouvel_objet_en_deplacement" addPublicVariableEventHandler R3F_LOG1_FNCT_PVEH_nouvel_objet_en_deplacement;
 ...
 */
R3F_LOG1_FNCT_PVEH_fin_deplacement_objet =
{
	private ["_objet"];
	
	_objet = _this select 1;
	
	R3F_LOG1_liste_objets_en_deplacement = R3F_LOG1_liste_objets_en_deplacement - [_this select 1];
...
"R3F_LOG1_PV_fin_deplacement_objet" addPublicVariableEventHandler R3F_LOG1_FNCT_PVEH_fin_deplacement_objet;
...
R3F_LOG1_FNCT_EH_HandleDamage =
{
	...
	if (
		...
		_this select 4 == "" && {(isNull _source || _source == _unite || _source in R3F_LOG1_liste_objets_en_deplacement)
		&& {
			// Si l'unité est potentiellement en collision avec un objet en cours de déplacement
			{
				!isNull _x &&
				{
					...
					] call R3F_LOG1_FNCT_3D_pos_est_dans_bbox
				}
			} count R3F_LOG1_liste_objets_en_deplacement != 0
		}
	}) then
	{
...
while {true} do
{
	...
	{
		// Unité non gérée
		if (isNil {_x getVariable "R3F_LOG1_idx_EH_HandleDamage"}) then
		{
			// Et qui est locale
			if (local _x) then
			{
				// Event handler de à chaque blessure, vérifiant si elle est due à un objet en déplacement
				_x setVariable ["R3F_LOG1_idx_EH_HandleDamage", _x addEventHandler ["HandleDamage", {_this call R3F_LOG1_FNCT_EH_HandleDamage}]];
			};
		}
		// Unité déjà gérée
		else
		{
			...
			if (!local _x) then
			{
				...
				_x removeEventHandler ["HandleDamage", _x getVariable "R3F_LOG1_idx_EH_HandleDamage"];
				_x setVariable ["R3F_LOG1_idx_EH_HandleDamage", nil];
			};
		};
	...
	for [{_idx_objet = 0}, {_idx_objet < count R3F_LOG1_liste_objets_en_deplacement}, {;}] do
	{
		private ["_objet"];
		
		_objet = R3F_LOG1_liste_objets_en_deplacement select _idx_objet;
		
		if (isNull _objet) then
		{
			R3F_LOG1_liste_objets_en_deplacement = R3F_LOG1_liste_objets_en_deplacement - [objNull];
		...
		else
		{
			...
			{
				["R3F_LOG1_PV_fin_deplacement_objet", _objet] call R3F_LOG1_FNCT_PVEH_fin_deplacement_objet;
