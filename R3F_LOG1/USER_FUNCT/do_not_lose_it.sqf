 ...
 * 
 * @usage nul = [my_object] execVM "R3F_LOG1\USER_FUNCT\do_not_lose_it.sqf";
 * @usage nul = [my_object, "spawn_pos"] execVM "R3F_LOG1\USER_FUNCT\do_not_lose_it.sqf";
 * @usage nul = [my_object, "cargo_pos"] execVM "R3F_LOG1\USER_FUNCT\do_not_lose_it.sqf";
 * @usage nul = [my_object, "<marker name>"] execVM "R3F_LOG1\USER_FUNCT\do_not_lose_it.sqf";
 * 
...
if (!isNull _objet) then
{
	...
	if (R3F_LOG1_active) then
	{
		...
		if (isServer) then
		{
			...
			R3F_LOG1_liste_objets_a_proteger pushBack _objet;
			
			R3F_LOG1_mutex_local_verrou = false;
			
			if (_pos_respawn == "spawn_pos" || _pos_respawn == "exact_spawn_pos") then
			{
				_objet setVariable ["R3F_LOG1_pos_respawn", getPosASL _objet, false];
				
				if (_pos_respawn == "exact_spawn_pos") then
				{
					_objet setVariable ["R3F_LOG1_dir_respawn", getDir _objet, false];
				};
			}
			else
			{
				_objet setVariable ["R3F_LOG1_pos_respawn", _pos_respawn, false];
			};
		};
	};
};