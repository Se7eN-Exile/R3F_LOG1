...
else
{
	...
	{
		if (
			_x != _objet && (_x getVariable ["R3F_LOG1_fonctionnalites", R3F_LOG1_CST_zero_log] select R3F_LOG1_IDX_can_tow) &&
			alive _x && isNull (_x getVariable "R3F_LOG1_est_transporte_par") &&
			isNull (_x getVariable "R3F_LOG1_remorque") && (vectorMagnitude velocity _x < 6) &&
			!([_x, player] call R3F_LOG1_FNCT_objet_est_verrouille) && !(_x getVariable "R3F_LOG1_disabled") &&
			{
				private ["_delta_pos"];
	...
	if (!isNull _remorqueur) then
	{
		...
		{
			[_remorqueur, player] call R3F_LOG1_FNCT_definir_proprietaire_verrou;
			
			_remorqueur setVariable ["R3F_LOG1_remorque", _objet, true];
			_objet setVariable ["R3F_LOG1_est_transporte_par", _remorqueur, true];
			...
			if (_objet isKindOf "StaticWeapon") then
			{
				...
				[_objet, "setDir", (getDir _objet)-_azimut_canon] call R3F_LOG1_FNCT_exec_commande_MP;
			};
