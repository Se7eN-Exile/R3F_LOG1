...
else
{
	...
	_remorqueur = [_objet, 5] call R3F_LOG1_FNCT_3D_cursorTarget_virtuel;
	
	if (!isNull _remorqueur && {
		_remorqueur getVariable ["R3F_LOG1_fonctionnalites", R3F_LOG1_CST_zero_log] select R3F_LOG1_IDX_can_tow &&
		alive _remorqueur && isNull (_remorqueur getVariable "R3F_LOG1_remorque") && (vectorMagnitude velocity _remorqueur < 6) && !(_remorqueur getVariable "R3F_LOG1_disabled")
	}) then
	{
