...
else
{
	...
	_duree = R3F_LOG1_CFG_unlock_objects_timer;
	...
	_ctrl_titre ctrlSetText format [STR_R3F_LOG1_deverrouillage_en_cours, _duree];
	...
	{
		_ctrl_titre ctrlSetText format [STR_R3F_LOG1_deverrouillage_en_cours, ceil (_duree - (time - _time_debut))];
	...
	if (_attente_valide) then
	{
		...
		systemChat STR_R3F_LOG1_deverrouillage_succes_attente;
	}
	else
	{
		hintC STR_R3F_LOG1_deverrouillage_echec_attente;
	};
