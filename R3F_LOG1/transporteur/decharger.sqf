...
else
{
	...
	_transporteur = uiNamespace getVariable "R3F_LOG1_dlg_CV_transporteur";
	_objets_charges = _transporteur getVariable ["R3F_LOG1_objets_charges", []];
	
	if (lbCurSel R3F_LOG1_IDC_dlg_CV_liste_contenu == -1) exitWith {R3F_LOG1_mutex_local_verrou = false;};
	
	_type_objet_a_decharger = lbData [R3F_LOG1_IDC_dlg_CV_liste_contenu, lbCurSel R3F_LOG1_IDC_dlg_CV_liste_contenu];
	
	_est_deplacable = ([_type_objet_a_decharger] call R3F_LOG1_FNCT_determiner_fonctionnalites_logistique) select R3F_LOG1_IDX_can_be_moved_by_player;
	...
	if (_action_confirmee) then
	{
		...
		{
			[_objet_a_decharger, player] call R3F_LOG1_FNCT_definir_proprietaire_verrou;
			...
			_objet_a_decharger setVariable ["R3F_LOG1_est_transporte_par", objNull, true];
			...
			if (!(_objet_a_decharger isKindOf "AllVehicles") || _est_deplacable) then
			{
				...
				[_objet_a_decharger, player, 0, true] spawn R3F_LOG1_FNCT_objet_deplacer;
			}
			else
			{
				private ["_bbox_dim", "_pos_degagee", "_rayon"];
				
				systemChat STR_R3F_LOG1_action_decharger_en_cours;
				...
				if (count _pos_degagee > 0) then
				{
					...
					{
						systemChat format [STR_R3F_LOG1_action_decharger_fait + " (%2)",
					...
					{
						systemChat format [STR_R3F_LOG1_action_decharger_fait, getText (configFile >> "CfgVehicles" >> (typeOf _objet_a_decharger) >> "displayName")];
					};
					R3F_LOG1_mutex_local_verrou = false;
				}
		...
		{
			hintC STR_R3F_LOG1_action_decharger_deja_fait;
			R3F_LOG1_mutex_local_verrou = false;
		};
	}
	else
