...
// Pour chaque élément de la liste à charger
{
	...
	if (typeName _objet_ou_classe == "STRING") then
	{
		...
		{
			_pos_degagee = [_bbox_dim, getPos _transporteur, 200, 50] call R3F_LOG1_FNCT_3D_tirer_position_degagee_sol;
		}
		else
		{
	...
	_cout_chargement_objet = _fonctionnalites select R3F_LOG1_IDX_can_be_transported_cargo_cout;
	
	// S'assurer que le type d'objet à charger est transportable
	if !(_fonctionnalites select R3F_LOG1_IDX_can_be_transported_cargo) then
	{
	...
	else
	{
		for [{_i = 0}, {_i < _quantite}, {_i = _i+1}] do
		{
			...
			if (!isNull _objet) then
			{
				...
				if (isNull (_objet getVariable ["R3F_LOG1_est_transporte_par", objNull]) &&
					(isNull (_objet getVariable ["R3F_LOG1_est_deplace_par", objNull]) || (!alive (_objet getVariable ["R3F_LOG1_est_deplace_par", objNull])) || (!isPlayer (_objet getVariable ["R3F_LOG1_est_deplace_par", objNull])))
				) then
				{
					if (isNull (_objet getVariable ["R3F_LOG1_remorque", objNull])) then
					{
						...
						else
						{
							diag_log format ["[Auto-load ""%1"" in ""%2""] : %3",
								getText (configFile >> "CfgVehicles" >> _classe >> "displayName"),
								getText (configFile >> "CfgVehicles" >> (typeOf _transporteur) >> "displayName"),
								STR_R3F_LOG1_action_charger_pas_assez_de_place];
					...
					{
						diag_log format [STR_R3F_LOG1_objet_remorque_en_cours, getText (configFile >> "CfgVehicles" >> _classe >> "displayName")];
						systemChat format [STR_R3F_LOG1_objet_remorque_en_cours, getText (configFile >> "CfgVehicles" >> _classe >> "displayName")];
					};
				}
				else
				{
					diag_log format [STR_R3F_LOG1_objet_en_cours_transport, getText (configFile >> "CfgVehicles" >> _classe >> "displayName")];
					systemChat format [STR_R3F_LOG1_objet_en_cours_transport, getText (configFile >> "CfgVehicles" >> _classe >> "displayName")];
				};
			};
		};
	};
