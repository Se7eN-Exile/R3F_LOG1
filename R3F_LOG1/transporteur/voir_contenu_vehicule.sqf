...
uiNamespace setVariable ["R3F_LOG1_dlg_CV_transporteur", _transporteur];

[_transporteur, player] call R3F_LOG1_FNCT_definir_proprietaire_verrou;

createDialog "R3F_LOG1_dlg_contenu_vehicule";
waitUntil (uiNamespace getVariable "R3F_LOG1_dlg_contenu_vehicule");
_dlg_contenu_vehicule = findDisplay R3F_LOG1_IDD_dlg_contenu_vehicule;

/**** DEBUT des traductions des labels ****/
(_dlg_contenu_vehicule displayCtrl R3F_LOG1_IDC_dlg_CV_titre) ctrlSetText STR_R3F_LOG1_dlg_CV_titre;
(_dlg_contenu_vehicule displayCtrl R3F_LOG1_IDC_dlg_CV_credits) ctrlSetText "[R3F] Logistics";
(_dlg_contenu_vehicule displayCtrl R3F_LOG1_IDC_dlg_CV_btn_decharger) ctrlSetText STR_R3F_LOG1_dlg_CV_btn_decharger;
(_dlg_contenu_vehicule displayCtrl R3F_LOG1_IDC_dlg_CV_btn_fermer) ctrlSetText STR_R3F_LOG1_dlg_CV_btn_fermer;
/**** FIN des traductions des labels ****/

_ctrl_liste = _dlg_contenu_vehicule displayCtrl R3F_LOG1_IDC_dlg_CV_liste_contenu;

_chargement_precedent = [];

while {!isNull _dlg_contenu_vehicule} do
{
	_chargement = [_transporteur] call R3F_LOG1_FNCT_calculer_chargement_vehicule;
	...
	if !([_chargement, _chargement_precedent] call BIS_fnc_areEqual) then
	{
		...
		_contenu = _transporteur getVariable ["R3F_LOG1_objets_charges", []];
		...
		for [{_i = 0}, {_i < count _contenu}, {_i = _i + 1}] do
		{
			...
			if !((typeOf _objet) in _tab_objets) then
			{
				...
				if (!isNil {_objet getVariable "R3F_LOG1_fonctionnalites"}) then
				{
					_tab_cout_chargement pushBack (_objet getVariable "R3F_LOG1_fonctionnalites" select R3F_LOG1_IDX_can_be_transported_cargo_cout);
				}
				else
				{
					_tab_cout_chargement pushBack (([typeOf _objet] call R3F_LOG1_FNCT_determiner_fonctionnalites_logistique) select R3F_LOG1_IDX_can_be_transported_cargo_cout);
				};
			}
			else
			{
		...
		lbClear _ctrl_liste;
		(_dlg_contenu_vehicule displayCtrl R3F_LOG1_IDC_dlg_CV_capacite_vehicule) ctrlSetText (format [STR_R3F_LOG1_dlg_CV_capacite_vehicule+" pl.", _chargement select 0, _chargement select 1]);
		if (_chargement select 1 != 0) then {(_dlg_contenu_vehicule displayCtrl R3F_LOG1_IDC_dlg_CV_jauge_chargement) progressSetPosition ((_chargement select 0) / (_chargement select 1));};
		(_dlg_contenu_vehicule displayCtrl R3F_LOG1_IDC_dlg_CV_jauge_chargement) ctrlShow ((_chargement select 0) != 0);
		
		if (count _tab_objets == 0) then
		{
			(_dlg_contenu_vehicule displayCtrl R3F_LOG1_IDC_dlg_CV_btn_decharger) ctrlEnable false;
		}
		else
		{
			...
			for [{_i = 0}, {_i < count _tab_objets}, {_i = _i + 1}] do
			{
				...
				if (uiNamespace getVariable ["R3F_LOG1_dlg_CV_lbCurSel_data", ""] == _classe) then
				{
					_ctrl_liste lbSetCurSel _index;
				};
			};
			
			(_dlg_contenu_vehicule displayCtrl R3F_LOG1_IDC_dlg_CV_btn_decharger) ctrlEnable true;
		};
	};
