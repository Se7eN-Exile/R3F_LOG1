...
else
{
	...
	{
		if (
			(_x getVariable ["R3F_LOG1_fonctionnalites", R3F_LOG1_CST_zero_log] select R3F_LOG1_IDX_can_be_lifted) &&
			_x != _heliporteur && !(_x getVariable "R3F_LOG1_disabled") &&
	...
	{
		if !(_objet getVariable "R3F_LOG1_disabled") then
		{
			if (isNull (_objet getVariable "R3F_LOG1_est_transporte_par") && (isNull (_objet getVariable "R3F_LOG1_est_deplace_par") || (!alive (_objet getVariable "R3F_LOG1_est_deplace_par")) || (!isPlayer (_objet getVariable "R3F_LOG1_est_deplace_par")))) then
			{
				...
				//{
					...
					if (isNull (_objet getVariable ["R3F_LOG1_remorque", objNull])) then
					{
						...
						_ctrl_titre ctrlSetText format [STR_R3F_LOG1_action_heliport_attente, _duree];
						...
						{
							_ctrl_titre ctrlSetText format [STR_R3F_LOG1_action_heliport_attente, ceil (_duree - (time - _time_debut))];
							...
							if !(
								alive player && vehicle player == _heliporteur && !(_heliporteur getVariable "R3F_LOG1_disabled") && _pas_de_hook &&
								isNull (_heliporteur getVariable "R3F_LOG1_heliporte") && (vectorMagnitude velocity _heliporteur < 6) && (_heliporteur distance _objet < 15) &&
								!(_objet getVariable "R3F_LOG1_disabled") && isNull (_objet getVariable "R3F_LOG1_est_transporte_par") &&
								(isNull (_objet getVariable "R3F_LOG1_est_deplace_par") || (!alive (_objet getVariable "R3F_LOG1_est_deplace_par")) || (!isPlayer (_objet getVariable "R3F_LOG1_est_deplace_par"))) &&
						...
						if (_attente_valide) then
						{
							...
							_heliporteur setVariable ["R3F_LOG1_heliporte", _objet, true];
							_objet setVariable ["R3F_LOG1_est_transporte_par", _heliporteur, true];
							...
							{
								[_objet, "setDir", 90] call R3F_LOG1_FNCT_exec_commande_MP;
							};
							
							systemChat format [STR_R3F_LOG1_action_heliporter_fait, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")];
							...
							[_heliporteur, _objet] spawn
							{
								...
								while {_heliporteur getVariable "R3F_LOG1_heliporte" == _objet} do
								{
						...
						{
							systemChat STR_R3F_LOG1_action_heliport_echec_attente;
					...
					{
						systemChat format [STR_R3F_LOG1_objet_remorque_en_cours, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")];
					};
				//}
				//else
				//{
				//	systemChat format [STR_R3F_LOG1_joueur_dans_objet, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")];
				//};
			}
			else
			{
				systemChat format [STR_R3F_LOG1_objet_en_cours_transport, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")];
			};
		};
	};
