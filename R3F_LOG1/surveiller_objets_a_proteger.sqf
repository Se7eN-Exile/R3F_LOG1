...
while {true} do
{
	// Pour chaque objet à protéger
	{
		...
		if (!isNull _objet) then
		{
			...
			if !(isNull (_objet getVariable ["R3F_LOG1_est_transporte_par", objNull])) then
			{
				...
				if !(alive (_objet getVariable "R3F_LOG1_est_transporte_par")) then
				{
					// Récupération de la position de respawn en accord avec le paramètre passé dans "do_not_lose_it"
					if (typeName (_objet getVariable "R3F_LOG1_pos_respawn") == "ARRAY") then
					{
						_pos_respawn = _objet getVariable "R3F_LOG1_pos_respawn";
					}
					else
					{
						if (_objet getVariable "R3F_LOG1_pos_respawn" == "cargo_pos") then
						{
							_pos_respawn = getPos (_objet getVariable "R3F_LOG1_est_transporte_par");
						}
						else
						{
							_pos_respawn = getMarkerPos (_objet getVariable "R3F_LOG1_pos_respawn");
						};
					};
					...
					if (isNil {_objet getVariable "R3F_LOG1_dir_respawn"}) then
					{
					...
					else
					{
						...
						_objet setDir (_objet getVariable "R3F_LOG1_dir_respawn");
					};
					
					// On retire l'objet du contenu du véhicule (s'il est dedans)
					_objets_charges = (_objet getVariable "R3F_LOG1_est_transporte_par") getVariable ["R3F_LOG1_objets_charges", []];
					if (_objet in _objets_charges) then
					{
						_objets_charges = _objets_charges - [_objet];
						(_objet getVariable "R3F_LOG1_est_transporte_par") setVariable ["R3F_LOG1_objets_charges", _objets_charges, true];
					};
	...
	} forEach R3F_LOG1_liste_objets_a_proteger;
