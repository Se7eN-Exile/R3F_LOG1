...
{
	if (_x in R3F_LOG1_objets_depl_heli_remorq_transp) exitWith {_can_be_depl_heli_remorq_transp = true;};
...
{
	{
		if (_x in R3F_LOG1_CFG_can_be_moved_by_player) exitWith {_can_be_moved_by_player = true;};
	} forEach _tab_classe_heritage;
	
	{
		if (_x in R3F_LOG1_CFG_can_be_lifted) exitWith {_can_be_lifted = true;};
	} forEach _tab_classe_heritage;
	
	{
		if (_x in R3F_LOG1_CFG_can_be_towed) exitWith {_can_be_towed = true;};
	} forEach _tab_classe_heritage;
	
	{
		_idx = R3F_LOG1_classes_objets_transportables find _x;
		if (_idx != -1) exitWith
		{
			_can_be_transported_cargo = true;
			_can_be_transported_cargo_cout = R3F_LOG1_CFG_can_be_transported_cargo select _idx select 1;
		};
	} forEach _tab_classe_heritage;
};
...
_can_lift = false;
{
	if (_x in R3F_LOG1_CFG_can_lift) exitWith {_can_lift = true;};
} forEach _tab_classe_heritage;

_can_tow = false;
{
	if (_x in R3F_LOG1_CFG_can_tow) exitWith {_can_tow = true;};
} forEach _tab_classe_heritage;

_can_transport_cargo = false;
_can_transport_cargo_cout = 0;
{
	_idx = R3F_LOG1_classes_transporteurs find _x;
	if (_idx != -1) exitWith
	{
		_can_transport_cargo = true;
		_can_transport_cargo_cout = R3F_LOG1_CFG_can_transport_cargo select _idx select 1;
	};
} forEach _tab_classe_heritage;
