 ...
 * R3F_LOG1_FNCT_3D_ray_intersect_bbox
 * R3F_LOG1_FNCT_3D_ray_intersect_bbox_obj
 * R3F_LOG1_FNCT_3D_cam_intersect_bbox_obj
 * 
 * R3F_LOG1_FNCT_3D_pos_est_dans_bbox
 * R3F_LOG1_FNCT_3D_distance_min_pos_bbox
 * 
 * R3F_LOG1_FNCT_3D_bounding_sphere_intersect_bounding_sphere
 * R3F_LOG1_FNCT_3D_intersect_bounding_sphere_objs
 * R3F_LOG1_FNCT_3D_bounding_sphere_intersect_bounding_box
 * R3F_LOG1_FNCT_3D_bbox_intersect_bbox
 * R3F_LOG1_FNCT_3D_bbox_intersect_bbox_objs
 * R3F_LOG1_FNCT_3D_mesh_collision_objs
 * 
 * // Recherche de position
 * R3F_LOG1_FNCT_3D_tirer_position_degagee_ciel
 * R3F_LOG1_FNCT_3D_tirer_position_degagee_sol
 * 
 * // Utilitaires
 * R3F_LOG1_FNCT_3D_cursorTarget_distance_bbox
 * R3F_LOG1_FNCT_3D_cursorTarget_virtuel
 * R3F_LOG1_FNCT_3D_get_huit_coins_bounding_box_model
 * R3F_LOG1_FNCT_3D_get_huit_coins_bounding_box_world
 * R3F_LOG1_FNCT_3D_get_objets_genants_rayon
 * R3F_LOG1_FNCT_3D_get_bounding_box_depuis_classname
 * R3F_LOG1_FNCT_3D_get_hauteur_terrain_min_max_objet
 * 
 * // Transformation 3D
 * R3F_LOG1_FNCT_3D_mult_mat3x3
 * R3F_LOG1_FNCT_3D_mult_vec_mat3x3
 * R3F_LOG1_FNCT_3D_mat_rot_roll
 * R3F_LOG1_FNCT_3D_mat_rot_pitch
 * R3F_LOG1_FNCT_3D_mat_rot_yaw
 * 
 * // Visualisation
 * R3F_LOG1_FNCT_3D_tracer_bbox
 * R3F_LOG1_FNCT_3D_tracer_bbox_obj
 */
...
R3F_LOG1_FNCT_3D_ray_intersect_bbox_obj =
{
	...
	] call R3F_LOG1_FNCT_3D_ray_intersect_bbox
};
...
R3F_LOG1_FNCT_3D_cam_intersect_bbox_obj =
{
	...
	[_pos_cam, _dir_cam, _objet] call R3F_LOG1_FNCT_3D_ray_intersect_bbox_obj
};
...
R3F_LOG1_FNCT_3D_intersect_bounding_sphere_objs =
{
	...
	// Valeurs selon le formule de R3F_LOG1_FNCT_3D_bounding_sphere_intersect_bounding_sphere
	//_pos1 = [0,0,0];
...
R3F_LOG1_FNCT_3D_bounding_sphere_intersect_bounding_box =
{
	...
	// Utilisation "inline" de la fonction R3F_LOG1_FNCT_3D_distance_min_pos_bbox
...
R3F_LOG1_FNCT_3D_bbox_intersect_bbox =
{
	...
	if !(
			[
				...
				_bbox2_min,
				_bbox2_max
			] call R3F_LOG1_FNCT_3D_bounding_sphere_intersect_bounding_box
		&&
			[
	...
	{
		// Utilisation "inline" de la fonction R3F_LOG1_FNCT_3D_pos_est_dans_bbox
		if (
	...
	// Test d'intersection de chaque rayon avec la bounding box de l'objet1
	{
		...
		{
			if ([_x select 0, _bbox1_min, _bbox1_max] call R3F_LOG1_FNCT_3D_pos_est_dans_bbox) exitWith {_intersect = true;};
		}
		else
		{
			if ([
				...
				_bbox1_min,
				_bbox1_max
			] call R3F_LOG1_FNCT_3D_ray_intersect_bbox <= (_x select 2)) exitWith {_intersect = true;};
		};
	} forEach _rayons;
...
R3F_LOG1_FNCT_3D_bbox_intersect_bbox_objs =
{
	...
	] call R3F_LOG1_FNCT_3D_bbox_intersect_bbox
};
...
R3F_LOG1_FNCT_3D_mesh_collision_objs =
{
	...
	_pos_test = ATLtoASL (player modelToWorld [0,16,20]);// TODO remplacer par R3F_LOG1_FNCT_3D_tirer_position_degagee_ciel
	
	_objet_test1 = (typeOf _objet1) createVehicleLocal ([] call R3F_LOG1_FNCT_3D_tirer_position_degagee_ciel);
	_objet_test1 setVectorDirAndUp [vectorDir _objet1, vectorUp _objet1];
	_objet_test1 allowDamage false;
	_objet_test1 addEventHandler ["EpeContactStart", {if (!isNull (_this select 1)) then {(_this select 0) setVariable ["R3F_LOG1_3D_collision", true, false];};}];
	_objet_test1 setVariable ["R3F_LOG1_3D_collision", false, false];
	
	_objet_test2 = (typeOf _objet2) createVehicleLocal ([] call R3F_LOG1_FNCT_3D_tirer_position_degagee_ciel);
	_objet_test2 setVectorDirAndUp [vectorDir _objet2, vectorUp _objet2];
	_objet_test2 allowDamage false;
	_objet_test2 addEventHandler ["EpeContactStart", {if (!isNull (_this select 1)) then {(_this select 0) setVariable ["R3F_LOG1_3D_collision", true, false];};}];
	_objet_test2 setVariable ["R3F_LOG1_3D_collision", false, false];
	...
	waitUntil
	{
		_collision = (_objet_test1 getVariable "R3F_LOG1_3D_collision") || (_objet_test2 getVariable "R3F_LOG1_3D_collision");
	...
	for [
		...
		},
		{
			...
			else
			{
				...
				} forEach ([_position_degagee, _rayon_degage+15] call R3F_LOG1_FNCT_3D_get_objets_genants_rayon);
	...
	if (!isNull _objet && !isNull _joueur && alive _joueur && cameraOn == _joueur) then
	{
		[
			...
			[
				...
				boundingBoxReal _objet select 0,
				boundingBoxReal _objet select 1
			] call R3F_LOG1_FNCT_3D_distance_min_pos_bbox
		]
	}
	else
	{
		...
		
		{
			if (!(_x in _liste_ingores) &&
				[
					...
					boundingBoxReal _x select 0,
					boundingBoxReal _x select 1
				] call R3F_LOG1_FNCT_3D_distance_min_pos_bbox <= _distance_max
...
R3F_LOG1_FNCT_3D_get_bounding_box_depuis_classname =
{
	...
	_objet_tmp = _classe createVehicleLocal ([] call R3F_LOG1_FNCT_3D_tirer_position_degagee_ciel);
	sleep 0.01;
...
R3F_LOG1_FNCT_3D_tracer_bbox =
{
	...
	if !(isNull _objet) then
	{
		...
		_coins = [_objet] call R3F_LOG1_FNCT_3D_get_huit_coins_bounding_box_world;
...
R3F_LOG1_FNCT_3D_tracer_bbox_obj =
{
	...
	if !(isNull _objet) then
	{
		[_objet, boundingBoxReal _objet select 0, boundingBoxReal _objet select 1] call R3F_LOG1_FNCT_3D_tracer_bbox;
	};
};

// Quelques contrôles et visualisations in-game durant le développement
//#define R3F_LOG1_3D_dev_mode // TODO commenter cette ligne lors du release
#ifdef R3F_LOG1_3D_dev_mode
if (isNil "R3F_LOG1_joueur_deplace_objet") then {R3F_LOG1_joueur_deplace_objet = objNull};
addMissionEventHandler ["Draw3D",
{
	if !(isNull player) then
	{
		...
		if (!isNull R3F_LOG1_joueur_deplace_objet) then
		{
			//[R3F_LOG1_joueur_deplace_objet] call R3F_LOG1_FNCT_3D_tracer_bbox_obj;
			
			{
				if ([
					R3F_LOG1_joueur_deplace_objet,
					boundingBoxReal R3F_LOG1_joueur_deplace_objet select 0,
					boundingBoxReal R3F_LOG1_joueur_deplace_objet select 1,
					_x,
					boundingBoxReal _x select 0,
					boundingBoxReal _x select 1
				] call R3F_LOG1_FNCT_3D_bbox_intersect_bbox) then
				{
					//systemChat format ["COLLISION BBOX %1 @ %2", typeOf _x, time];
					[_x] call R3F_LOG1_FNCT_3D_tracer_bbox_obj;
				};
			} forEach (nearestObjects [R3F_LOG1_joueur_deplace_objet, ["All"], 15] - [player, R3F_LOG1_joueur_deplace_objet]);
		}
		else
		{
			if (false && !isNull _objet) then
			{
				hintSilent format ["%1 | %2 | %3", typeOf _objet, [_objet] call R3F_LOG1_FNCT_3D_cam_intersect_bbox_obj,
					[_objet worldToModel (positionCameraToWorld [0, 0, 0]), boundingBoxReal _objet select 0, boundingBoxReal _objet select 1] call R3F_LOG1_FNCT_3D_pos_est_dans_bbox];
				
				[_objet] call R3F_LOG1_FNCT_3D_tracer_bbox_obj;
			}
			else
			{
				_cursorTarget_distance = call R3F_LOG1_FNCT_3D_cursorTarget_distance_bbox;
				...
				{
					[_cursorTarget_distance select 0] call R3F_LOG1_FNCT_3D_tracer_bbox_obj;
				};
			};
		};
		...
		//	
		//	if ([_x worldToModel (player modelToWorld [0,0,0]), _bbox_min_elargie, _bbox_max_elargie] call R3F_LOG1_FNCT_3D_pos_est_dans_bbox) then
		//	{
		...
		//if !(isNil "R3F_LOG1_liste_objets_en_deplacement") then
		//{
		//	{
		//		[_x] call R3F_LOG1_FNCT_3D_tracer_bbox_obj;
		//	} forEach R3F_LOG1_liste_objets_en_deplacement;
		//};
		
		//{
		//	[_x] call R3F_LOG1_FNCT_3D_tracer_bbox_obj;
		//} forEach [[[R3F_LOG1_joueur_deplace_objet]] call R3F_LOG1_FNCT_3D_cursorTarget_virtuel];
		
		//{
		//	[_x] call R3F_LOG1_FNCT_3D_tracer_bbox_obj;
		//} forEach ([player, 100] call R3F_LOG1_FNCT_3D_get_objets_genants_rayon);
	};
}];
#endif