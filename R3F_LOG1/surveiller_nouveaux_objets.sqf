...
while {true} do
{
	if (!isNull player) then
	{
		...
		if (_compteur_cyclique == 0 || R3F_LOG1_PUBVAR_nouvel_objet_a_initialiser) then
		{
			R3F_LOG1_PUBVAR_nouvel_objet_a_initialiser = false; // Acquittement local
			...
			if (count _liste_statiques_connus > _seuil_nb_statiques_avant_purge) then
			{
				_liste_purge = [];
				{
					if (!isNull _x &&
						{
							!isNil {_x getVariable "R3F_LOG1_fonctionnalites"} ||
							(_x getVariable ["R3F_LOG1_CF_depuis_usine", false])
						}
					) then
					{
						_liste_purge pushBack _x;
		...
		if (_count_liste_objets > 0) then
		{
			...
			for [{_i = 0}, {_i < _count_liste_objets}, {_i = _i + 1}] do
			{
				...
				_fonctionnalites = [typeOf _objet] call R3F_LOG1_FNCT_determiner_fonctionnalites_logistique;
				...
				{
					_objet setVariable ["R3F_LOG1_fonctionnalites", _fonctionnalites, false];
					
					if (isNil {_objet getVariable "R3F_LOG1_disabled"}) then
					{
						_objet setVariable ["R3F_LOG1_disabled", R3F_LOG1_CFG_disabled_by_default, false];
					};
				...
				if (_objet getVariable ["R3F_LOG1_CF_depuis_usine", false]) then
				{
					_objet addAction [("<t color=""#ff9600"">" + format [STR_R3F_LOG1_action_revendre_usine_direct, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")] + "</t>"), {_this call R3F_LOG1_FNCT_usine_revendre_direct}, nil, 5, false, true, "", "!R3F_LOG1_mutex_local_verrou && R3F_LOG1_objet_addAction == _target && R3F_LOG1_action_revendre_usine_direct_valide"];
				};
