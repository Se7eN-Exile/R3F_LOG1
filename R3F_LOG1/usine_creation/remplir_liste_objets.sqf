...
_ctrl_liste_objets = _dlg_liste_objets displayCtrl R3F_LOG1_IDC_dlg_LO_liste_objets;

_sel_categorie = lbCurSel (_dlg_liste_objets displayCtrl R3F_LOG1_IDC_dlg_LO_liste_categories);
_sel_objet = 0 max (_usine getVariable "R3F_LOG1_CF_mem_idx_objet");
_usine setVariable ["R3F_LOG1_CF_mem_idx_objet", 0];

lbClear _ctrl_liste_objets;

if (count (_usine getVariable "R3F_LOG1_CF_cfgVehicles_par_categories") > 0) then
{
	// Insertion de chaque type d'objets disponibles dans la liste
	{
		...
		_index = _ctrl_liste_objets lbAdd format ["%1 (%2 cred.)", _nom, [_cout] call R3F_LOG1_FNCT_formater_nombre_entier_milliers];
		_ctrl_liste_objets lbSetPicture [_index, _icone];
		_ctrl_liste_objets lbSetData [_index, _classe];
	} forEach (_usine getVariable "R3F_LOG1_CF_cfgVehicles_par_categories" select _sel_categorie);
