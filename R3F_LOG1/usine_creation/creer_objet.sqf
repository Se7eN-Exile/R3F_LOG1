...
else
{
	...
	_usine = uiNamespace getVariable "R3F_LOG1_dlg_LO_usine";
	
	if (lbCurSel R3F_LOG1_IDC_dlg_LO_liste_objets == -1) exitWith {R3F_LOG1_mutex_local_verrou = false;};
	_classe = lbData [R3F_LOG1_IDC_dlg_LO_liste_objets, lbCurSel R3F_LOG1_IDC_dlg_LO_liste_objets];
	
	if (_classe != "") then
	{
		_cout = [_classe] call R3F_LOG1_FNCT_determiner_cout_creation;
		_est_deplacable = ([_classe] call R3F_LOG1_FNCT_determiner_fonctionnalites_logistique) select R3F_LOG1_IDX_can_be_moved_by_player;
		
		// L'usine a-t-elle assez de crédits ?
		if (_usine getVariable "R3F_LOG1_CF_credits" == -1 || _usine getVariable "R3F_LOG1_CF_credits" >= _cout) then
		{
			...
			if (_classe isKindOf "AllVehicles") then
			{
				...
				systemChat STR_R3F_LOG1_action_creer_en_cours;
				sleep 0.5;
				
				_bbox = [_classe] call R3F_LOG1_FNCT_3D_get_bounding_box_depuis_classname;
				...
				for [{_rayon = 5 max (2*_bbox_dim); _pos_degagee = [];}, {count _pos_degagee == 0 && _rayon <= 30 + (8*_bbox_dim)}, {_rayon = _rayon + 10 + (2*_bbox_dim)}] do
				{
					...
					] call R3F_LOG1_FNCT_3D_tirer_position_degagee_sol;
				};
			}
			else
			{
				_pos_degagee = [] call R3F_LOG1_FNCT_3D_tirer_position_degagee_ciel;
			};
			...
			if (count _pos_degagee > 0) then
			{
				...
				{
					_action_confirmee = [STR_R3F_LOG1_action_decharger_deplacable_exceptionnel, "Warning", true, true] call BIS_fnc_GUImessage;
				}
				else
				{
				...
				if (_action_confirmee) then
				{
					...
					if (_usine getVariable "R3F_LOG1_CF_credits" != -1) then
					{
						_usine setVariable ["R3F_LOG1_CF_credits", 0 max ((_usine getVariable "R3F_LOG1_CF_credits") - _cout), true];
					};
					...
					if !(isNull _objet) then
					{
						...
						(findDisplay R3F_LOG1_IDD_dlg_liste_objets displayCtrl R3F_LOG1_IDC_dlg_LO_btn_fermer) ctrlEnable false;
						
						// Mémoriser que cet objet a été créé depuis une usine
						_objet setVariable ["R3F_LOG1_CF_depuis_usine", true, true];
						
						[_objet, player] call R3F_LOG1_FNCT_definir_proprietaire_verrou;
						
						sleep 0.5;
						
						// Informer tout le monde qu'il y a un nouvel objet
						R3F_LOG1_PUBVAR_nouvel_objet_a_initialiser = true;
						publicVariable "R3F_LOG1_PUBVAR_nouvel_objet_a_initialiser";
						
						// Prise en compte de l'objet dans l'environnement du joueur (accélérer le retour des addActions)
						_objet spawn
						{
							sleep 4;
							R3F_LOG1_PUBVAR_reveler_au_joueur = _this;
							publicVariable "R3F_LOG1_PUBVAR_reveler_au_joueur";
							["R3F_LOG1_PUBVAR_reveler_au_joueur", R3F_LOG1_PUBVAR_reveler_au_joueur] spawn R3F_LOG1_FNCT_PUBVAR_reveler_au_joueur;
						};
						...
						if (!(_objet isKindOf "AllVehicles") || _est_deplacable) then
						{
							...
							[_objet, player, 0, true] spawn R3F_LOG1_FNCT_objet_deplacer;
						}
						else
						{
							...
							{
								systemChat format [STR_R3F_LOG1_action_creer_fait + " (%2)",
							...
							{
								systemChat format [STR_R3F_LOG1_action_creer_fait, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")];
							};
							R3F_LOG1_mutex_local_verrou = false;
						};
		...
		{
			hintC STR_R3F_LOG1_action_creer_pas_assez_credits;
			R3F_LOG1_mutex_local_verrou = false;
		};
	}
	else {R3F_LOG1_mutex_local_verrou = false;};
};