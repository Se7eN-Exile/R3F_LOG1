...
for [{_idx_config = 0}, {_idx_config < _nb_config}, {_idx_config = _idx_config+1}] do
{
	...
	if (isClass _config) then
	{
		...
		if (isNull _usine || {(_usine getVariable "R3F_LOG1_CF_blackwhitelist_mode" == "white") isEqualTo (toLower configName _config in (_usine getVariable "R3F_LOG1_CF_blackwhitelist_categories"))}) then
		{
...
for [{_idx_config = 0}, {_idx_config < _nb_config}, {_idx_config = _idx_config+1}] do
{
	...
	if (isClass _config) then
	{
		...
		if (getNumber (_config >> "scope") == 2 && !(configName _config isKindOf "Man")) then
		{
			...
			if (isNull _usine || {getNumber (_config >> "side") in (_usine getVariable "R3F_LOG1_CF_num_sides")}) then
			{
